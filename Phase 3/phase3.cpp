...
// 1-bit branch predictor table (assume 1024 entries)
const int BP_TABLE_SIZE = 1024;
bool branch_predictor[BP_TABLE_SIZE] = {false}; // false = not taken, true = taken

// Hash function to index into table (simple modulo)
inline int bp_index(int pc)
{
    return (pc >> 2) % BP_TABLE_SIZE;
}
...
void extract_sb(int IR, int index)
{
    ...
    if ((Control_line[index] >= 52 && Control_line[index] <= 57))
    {
        ...
        int index = bp_index(PC - 4); // since PC already incremented
        bool prediction = branch_predictor[index];

        // Actual branch outcome
        bool actual_taken = false;
        switch (Control_line[index])
        {
        case 52:
            actual_taken = (Ra == Rb);
            break; // beq
        ...
        case 55:
            ...
            break; // bge
        case 56:
            actual_taken = ((unsigned)Ra < (unsigned)Rb);
            break; // bltu
        case 57:
            actual_taken = ((unsigned)Ra >= (unsigned)Rb);
            break; // bgeu
        ...
        branch_predictor[index] = actual_taken;
...
{
    // only for pipelining , data forwarding
    if (knob2 == 1)
    {
